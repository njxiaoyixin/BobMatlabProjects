...
% from the sample nanmedian. Infinite elements are discarded if rep is 0, or
...
while outlier
	tmp = b(~isnan(b));
	meanval = nanmedian(tmp);
	maxval = tmp(find(abs(tmp-nanmedian(tmp))==max(abs(tmp-nanmedian(tmp)))));
	maxval = maxval(1);
	sdval = nanstd(tmp);
