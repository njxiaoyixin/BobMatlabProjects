...
for Date = SDate:EDate
    [Flag,Pos] = ismember(Date-1,TradeDay);
    if ~Flag%由于是凌晨更新，如果前一天不是交易日，则休息
        TitleStr= '现在是假期，电脑要休息啦！';
        disp(TitleStr)
        fprintf(fid,'%s\n',TitleStr);
        continue
    end
    ThisSDate = TradeDay(Pos)+15/24+1/60/60/24;%从前一个交易日下午15:00:01开始
    ThisEDate = TradeDay(Pos+1)+15/24;%到第二个交易日15：00结束
