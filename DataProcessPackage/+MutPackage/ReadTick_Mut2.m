classdef ReadTick_Mut2 < WindPackage.ReadTick
    % 设定国内期货交易时间，填充空白的格子
    methods
        function Data3 = F_LoadIntradayBar(~,Data,Interval)
            ...
            TimeSeries = [9/24:(Interval/24/60):(10+0.25)/24,(10+0.5)/24:(Interval/24/60):(11+0.5)/24,(13+0.5)/24:(Interval/24/60):15/24]';
...
%             TimeSeries = (0:(Interval/24/60):1)';
            ...
            %将没有数据的TimeSlot用之前的数据填充
            Fields  = fieldnames(Data2);
            for i=1:numel(Fields)
                ThisDataSpot = nan;
                Data3.(Fields{i}) = Data2.(Fields{i});
                for j=1:numel(Data2.Close)
                    if ~isnan(Data2.Close(j)) && ~isempty(Data2.Close(j))
                        ThisDataSpot = Data2.Close(j);
                    end
                    Data3.(Fields{i})(j) = ThisDataSpot;
                end                
            end
            %将没有数据的TimeSlot扔掉
